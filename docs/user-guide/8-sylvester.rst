...
Before publishing this page, verify the following against the current
production environment:

- current Sylvester queue names and eligibility;
- queue runtime, CPU, memory, concurrency, and preemption limits;
- current Sylvester project and dashboard request URLs;
- current storage quotas and pricing;
- whether the utilization dashboard URL is still externally reachable; and
- the canonical internal link for the current Nextflow documentation.

The retired hardware inventory from the previous Sylvester QuickStart should
not be restored here. Cluster-wide hardware information belongs in the
canonical Pegasus/Triton documentation, while any Sylvester-specific hardware
inventory should be maintained in one authoritative resource page if it is
still needed.
